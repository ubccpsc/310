...
As with D1, functional correctness comprises 80% of your grade and coverage comprises 20%. Unlike D1, 25% of the functional correctness score will be derived from private tests; you will not see any output for these private tests until after the final D2 deadline has passed.
...
The dataset file can be found here: [rooms.zip](https://github.com/ubccpsc/310/raw/2018sept/project/rooms.zip).
...
We are making one change to the ```courses``` dataset:

* **courses_year**: ```number```; Now, if the ``` "Section" ``` property in the source data is set to ```"overall"```, the year for that section should be set to 1900.
